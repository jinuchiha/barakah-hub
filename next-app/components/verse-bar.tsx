...
export function VerseBar({ locale = 'en' }: { locale?: 'ur' | 'en' }) {
  ...
  const v = VERSES[i];
  return (
    <div className="flex shrink-0 items-center justify-center gap-3 border-b border-[rgba(27,107,69,0.4)] bg-gradient-to-r from-[var(--color-emerald-3)] via-[#0f4a30] to-[var(--color-emerald-3)] px-6 py-1.5">
      ...
      <span className="text-xs text-white/20">·</span>
      <span className="font-[var(--font-en)] text-[10px] tracking-[1px] text-[rgba(27,107,69,0.85)]">{v.ref}</span>
    </div>
  );
}
